...
import com.authguidance.mobileweb.plumbing.oauth.logout.CognitoLogoutUrlBuilder
import com.authguidance.mobileweb.plumbing.oauth.logout.LogoutUrlBuilder
import com.authguidance.mobileweb.plumbing.oauth.logout.StandardLogoutUrlBuilder
import com.authguidance.mobileweb.plumbing.utilities.ConcurrentActionHandler
...
class AuthenticatorImpl(val configuration: OAuthConfiguration, val applicationContext: Context) : Authenticator {
                ...
                AuthorizationServiceConfiguration.RetrieveConfigurationCallback { serviceConfiguration, ex ->
                    when {
                        ...
                        serviceConfiguration == null -> {
                            val error = IllegalStateException("Metadata request returned an empty response")
    ...
    private suspend fun performLoginRedirect(activity: Activity, completionCode: Int) {
        ...
        try {
            ...
            // If configured, set the idp parameter to tell the Authorization Server which identity provider to use
            if (!this.configuration.idpParameterValue.isNullOrBlank()) {
                builder.setAdditionalParameters(
                    mapOf(this.configuration.idpParameterName to this.configuration.idpParameterValue)
                )
            }
            ...
            val callback =
                AuthorizationService.TokenResponseCallback { tokenResponse, ex ->
                    ...
                    when {
                        ...
                        // Sanity check
                        tokenResponse == null -> {
                            val empty = IllegalStateException("Authorization code grant returned an empty response")
            ...
            val callback =
                AuthorizationService.TokenResponseCallback { tokenResponse, ex ->
                    ...
                    when {
                        ...
                        // Sanity check
                        tokenResponse == null -> {
                            val error = IllegalStateException("Refresh token grant returned an empty response")
                            continuation.resumeWithException(error)
                            this.concurrencyHandler.resumeWithException(error)
                        }
    ...
    private fun getLoginRedirectUri(): String {
        return "${this.configuration.interstitialBaseUrl}${this.configuration.loginRedirectPath}"
    }
    ...
    private fun getPostLogoutRedirectUri(): String {
        return "${this.configuration.interstitialBaseUrl}${this.configuration.postLogoutRedirectPath}"
    }
